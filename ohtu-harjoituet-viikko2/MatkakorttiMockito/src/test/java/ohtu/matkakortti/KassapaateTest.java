...
public class KassapaateTest {
    ...
    public void kortiltaEiVelotetaJosRahaEiRiita() {
        ...
        @Test
    public void kassanMetodiLataaLisääKortilleRahamääränKunSummaPositiivinen() {
        when(kortti.getSaldo()).thenReturn(0);
        kassa.lataa(kortti, 5);
        
        verify(kortti, times(1)).lataa(5);
        verify(kortti).lataa(eq(Kassapaate.HINTA));

    }
            @Test
    public void kassanMetodiLataaLisääKortilleRahamääränKunSummanegatiivinen() {
        when(kortti.getSaldo()).thenReturn(0);
        kassa.lataa(kortti, -5);
